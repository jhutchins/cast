...
class CastPlugin(private val activity: Activity, private val channel: MethodChannel) : MethodCallHandler {
    ...
    private var _castSession: CastSession? = null
    private var _playbackClient: RemotePlaybackClient? = null
    private var _mediaRouter: MediaRouter = MediaRouter.getInstance(activity)
    private val _mediaRouterCallback = DidisoftMediaRouterCallback()
    private var _mediaRouteSelector: MediaRouteSelector? = null
    private var _sessionManagerListener: SessionManagerListener<CastSession>? = null
    ...
    private fun initChromecast(result: Result, app_id: String) {
        ...
        _castContext = CastContext.getSharedInstance(activity)

        _sessionManagerListener = object : SessionManagerListener<CastSession> {
        ...
        _castSession = _castContext?.sessionManager?.currentCastSession
        _castContext?.sessionManager?.addSessionManagerListener(_sessionManagerListener, CastSession::class.java)
    ...
    private fun play(result: Result, url: String, mimeType: String, extra: Map<String, String>?, position: Long) {

        val custom = JSONObject()
        extra?.forEach { k, v -> custom.put(k, v) }
        Log.d(TAG, "Start playing media\n\turl: $url\n\tmimetype: $mimeType\n\textra: $extra\n\tposition: $position")
        ...
        val media = MediaInfo.Builder(url)
                ...
                .build()
        Log.d(TAG, "Attempting play session = $_castSession, client = ${_castSession?.remoteMediaClient}")

        _castSession?.remoteMediaClient?.load(media, options)?.setResultCallback { result ->
            Log.i(TAG, "Playback status = " + result.status.statusMessage)
        }
...
//        })
    ...
    private fun disposeChromecast(result: Result) {
        _mediaRouter.removeCallback(_mediaRouterCallback)
        _castContext?.sessionManager?.removeSessionManagerListener(_sessionManagerListener, CastSession::class.java)
